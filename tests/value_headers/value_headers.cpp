...
// "detail/stills_FFmpeg.h is the single libav include point, so the value headers stay FFmpeg-free and
// can appear in a consumer's own public headers." Nothing tested it — every other TU in the suite
// includes stills/stills_Stills.h, which pulls in everything. This one is built as a target that neither
...
// the one thing stills_Geometry.h would ever want from libav. Adding <libavutil/display.h> to
// stills_Geometry.h compiles this TU green.
//
...
#if defined(LIBAVFORMAT_VERSION_MAJOR) || defined(LIBAVCODEC_VERSION_MAJOR) || defined(LIBAVUTIL_VERSION_MAJOR)        \
    || defined(LIBSWSCALE_VERSION_MAJOR) || defined(AV_NOPTS_VALUE)
#error                                                                                                                 \
    "a value header reached a libav header: stills_AssetInfo/stills_Error/stills_Geometry/stills_Options/stills_PixelFormat/stills_Time/stills_Version must depend on detail/stills_FFmpeg.h through nothing. See README, 'Design decisions'."
#endif
...
static_assert (stills::Tolerance::exact().isExact());
static_assert (stills::Size{ 4, 0 }.isEmpty());
static_assert (stills::Size{ 4, 2 }.transposed() == stills::Size{ 2, 4 });
static_assert (stills::toString (stills::PixelFormat::rgba) == "rgba");
static_assert (stills::toString (stills::HardwarePolicy::automatic) == "automatic");
static_assert (stills::toString (stills::ErrorCode::decodeFailed) == "decodeFailed");
...
[[maybe_unused]] bool valueHeadersCompose()
{
    const stills::AssetInfo info;
    const stills::Options options;
    const stills::RequestOptions request;
    return ! info.getTimeRange().has_value() && options.validate().has_value()
           && request.validate (options.pixelFormat).has_value();
}
