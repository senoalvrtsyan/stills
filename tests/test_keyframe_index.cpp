...
AVPacket keyPacket (std::int64_t pts, std::int64_t dts, std::int64_t pos)
{
    AVPacket p{};
...
TEST_CASE ("KeyframeIndex: a recorded keyframe covers nothing until its GOP extent is known", "[keyframeIndex]")
{
    KeyframeIndex idx;
...
TEST_CASE ("KeyframeIndex: a reposition breaks the chain, so no extent is invented across it", "[keyframeIndex]")
{
    KeyframeIndex idx;
    idx.recordKey (0, 0, 100, scanned);
...
TEST_CASE ("KeyframeIndex: recording out of order keeps earlier extents intact", "[keyframeIndex]")
{
    KeyframeIndex idx;
...
TEST_CASE ("KeyframeIndex: a keyframe read a second time is updated, not duplicated", "[keyframeIndex]")
{
    KeyframeIndex idx;
    idx.recordKey (1000, 1000, 10, scanned);
...
TEST_CASE ("KeyframeIndex: the GOP estimate is a lower bound that only grows", "[keyframeIndex]")
{
    KeyframeIndex idx;
...
TEST_CASE ("KeyframeIndex: the reorder delay is the smallest a keyframe has shown", "[keyframeIndex]")
{
    KeyframeIndex idx;
    ...
    const AVPacket a = keyPacket (140, 100, 0);
    ...
    const AVPacket b = keyPacket (1040, 1020, 10);
    idx.notePacket (b, scanned);
    CHECK (idx.getReorderTicks() == 20);
    const AVPacket c = keyPacket (2100, 2000, 20);
    idx.notePacket (c, scanned);
    CHECK (idx.getReorderTicks() == 20);
}

TEST_CASE ("KeyframeIndex: packets that say nothing about keyframes are ignored", "[keyframeIndex]")
{
    KeyframeIndex idx;
    AVPacket nonkey{};
    nonkey.pts = 500;
    ...
    const AVPacket backwards = keyPacket (100, 140, 0);
...
TEST_CASE ("KeyframeIndex: a trusted container index is not shadowed by a recorded one", "[keyframeIndex]")
{
    KeyframeIndex idx;
    const AVPacket a = keyPacket (140, 100, 0);
...
TEST_CASE ("KeyframeIndex: a stream that ends inside a GOP bounds it at infinity", "[keyframeIndex]")
{
    KeyframeIndex idx;
    idx.recordKey (1000, 1000, 10, scanned);
...
TEST_CASE ("KeyframeIndex: queries before the first keyframe find nothing", "[keyframeIndex]")
{
    KeyframeIndex empty;
...
TEST_CASE ("KeyframeIndex: doesKeyCover and hasKeyBetween without a container index", "[keyframeIndex]")
{
    KeyframeIndex idx;
    idx.recordKey (1000, 1000, 10, scanned);
    idx.recordKey (2000, 2000, 20, scanned);
