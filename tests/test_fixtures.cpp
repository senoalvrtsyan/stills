...
struct Invariant {
  ...
  std::vector<int>
      keyframes;  // positions in the presented sequence that are keyframes, empty = not checked
  bool available = true;
};
  ...
  for (const Invariant& f : all) {
    ...
    DYNAMIC_SECTION(f.name) {
      ...
      for (int n = 0; n < 200; ++n) {
        ...
        if (distinct.empty() || distinct.back() != img->actual_time()) {
          if (img->is_keyframe()) keyframe_positions.push_back(static_cast<int>(distinct.size()));
          distinct.push_back(img->actual_time());
        }
      }
      INFO("presented frames: " << distinct.size() << ", keyframes: " << keyframe_positions.size());
      if (f.frames != 0) CHECK(static_cast<int>(distinct.size()) == f.frames);
      if (!f.keyframes.empty()) CHECK(keyframe_positions == f.keyframes);
      ...
      const int first_index = frame_index_of(first);
      CHECK(first_index >= 0);
      auto later = REQUIRE_OK(g.image_at(distinct.at(distinct.size() / 2)));
      CHECK(frame_index_of(later) > first_index);
    }
  }
}
...
TEST_CASE("fixtures: counter.mp4 carries the B-frame reordering the tests assume", "[fixtures]") {
  auto g = REQUIRE_OK(AssetImageGenerator::open(fixture("counter.mp4").string(), sw_options()));
  ...
  CHECK(info.time_base == stills::Rational{1, 15360});  // frame k is stored at pts k * 512
  ...
  CHECK(img.actual_time() == Time{47, 30});
}
