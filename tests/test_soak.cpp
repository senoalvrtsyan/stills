...
Options soakOptions (std::mt19937& rng)
{
    Options o = swOptions (rng() % 3 == 0 ? stills::PixelFormat::rgba : stills::PixelFormat::yuv420p);

    if (rng() % 4 == 0) o.tolerance = Tolerance::any();
    return o;
}

std::unique_ptr<AssetImageGenerator> openOne (std::mt19937& rng, int slot)
{
    const char* name =
        kFixtures[(static_cast<std::size_t> (slot) + rng() % std::size (kFixtures)) % std::size (kFixtures)];
    auto g = AssetImageGenerator::open (fixture (name).string(), soakOptions (rng));
...
struct Totals
{
    ...
    std::atomic<long> openFailures{ 0 };
    std::atomic<long> miscounted{ 0 };
    std::atomic<long> firstBadExpected{ 0 }, firstBadGot{ 0 };
};

} // namespace

TEST_CASE ("soak: eight generators, eight threads, destroyed and reopened underneath", "[soak][async][lifetime]")
{
    ...
    Totals t;
    {
        ...
        {
            slots[i].generator = openOne (rng, i);
    ...
    const auto worker = [&] (int id)
    {
        ...
        while (std::chrono::steady_clock::now() < end)
        {
            ...
            { // synchronous request
                ...
                if (s.generator->imageAt (Time{ static_cast<int> (rng() % 125), 30 }))
                {
                    ++t.images;
                }
                else
            ...
            { // a batch, sometimes cancelled, whose handler re-enters the generator
                std::shared_lock lk (s.mutex);
                ...
                for (int i = 0; i < n; ++i)
                    times.push_back (Time{ static_cast<int> (rng() % 125), 30 });
                ...
                auto req = s.generator->generateImages (times,
                                                        [track, g, reenter] (Completion c)
                                                        {
                                                            ++track->delivered;

                                                            // Documented as permitted: a handler may call back into
                                                            // its own generator.
                                                            if (reenter && c.index == 0)
                                                            {
                                                                (void)g->imageAt (Time{ 5, 30 });
                                                                g->cancelAll();
                                                            }
                                                        });
            ...
            for (auto it = pending.begin(); it != pending.end();)
            {
                ...
                if (got != static_cast<int> (it->second->expected) && got != 0)
                {
                    ...
                    {
                        t.firstBadExpected.store (static_cast<long> (it->second->expected));
                        t.firstBadGot.store (got);
                    }
                }
        ...
        for (auto& p : pending)
            (void)p.first.waitFor (std::chrono::seconds{ 30 });
    };
    ...
    INFO ("images " << t.images << " errors " << t.errors << " batches " << t.batches << " completions "
                    << t.completions << " reopens " << t.reopens);
    INFO ("first miscounted batch: expected " << t.firstBadExpected << " completions, got " << t.firstBadGot);
    CHECK (t.miscounted == 0);
    CHECK (t.openFailures == 0);
