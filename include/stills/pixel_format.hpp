#pragma once
// stills/pixel_format.hpp — output pixel formats the library can produce. No FFmpeg dependency;
// the mapping to AVPixelFormat lives in <stills/interop.hpp>.
...
enum class ColorRange : std::uint8_t { unspecified, limited, full };

[[nodiscard]] constexpr std::string_view to_string(PixelFormat f) noexcept {
  switch (f) {
...
[[nodiscard]] constexpr std::string_view to_string(ColorRange r) noexcept {
  switch (r) {
...
[[nodiscard]] constexpr int plane_count(PixelFormat f) noexcept {
  switch (f) {
    case PixelFormat::nv12:
    case PixelFormat::p010:
      return 2;
...
[[nodiscard]] constexpr bool is_packed_rgb(PixelFormat f) noexcept {
  return f != PixelFormat::gray8 && f != PixelFormat::nv12 && f != PixelFormat::yuv420p &&
         f != PixelFormat::p010;
}

/// True when the format stores chroma at half resolution (output dimensions must be even).
[[nodiscard]] constexpr bool has_chroma_subsampling(PixelFormat f) noexcept {
...
[[nodiscard]] constexpr int bit_depth(PixelFormat f) noexcept {
  switch (f) {
    case PixelFormat::p010:
      return 10;
    case PixelFormat::rgba64:
...
/// nv12).
[[nodiscard]] constexpr int bytes_per_pixel(PixelFormat f, int plane = 0) noexcept {
  switch (f) {
    case PixelFormat::rgba:
    case PixelFormat::bgra:
    case PixelFormat::argb:
...
[[nodiscard]] constexpr int chroma_shift(PixelFormat f) noexcept {
  return has_chroma_subsampling(f) ? 1 : 0;
}
...
#define STILLS_DEFINE_ENUM_FORMATTER(Enum)                                            \
  ...
  struct std::formatter<Enum> : std::formatter<std::string_view> {                    \
    ...
    auto format(Enum value, Ctx& ctx) const {                                         \
      return std::formatter<std::string_view>::format(stills::to_string(value), ctx); \
    }                                                                                 \
  }
