#pragma once
// stills/detail/keyframe_index.hpp — where the keyframes are, and how far a GOP reaches.
//
...
struct ContainerIndex {
  ...
  bool trusted{false};        ///< StreamInfo::index_trusted
  std::int64_t frameDurationHint{0};  ///< StreamInfo::frame_duration_hint; 0 = unknown
};
...
class KeyframeIndex {
 public:
  ...
  void recordKey(std::int64_t pts, std::int64_t dts, std::int64_t pos,
                 const ContainerIndex& container) {
    if (pts == k::no_pts || pos < 0 || container.trusted) return;
    std::size_t i = lowerBound(pts);
    if (i < entries.size() && entries[i].pts == pts) {
      entries[i].dts = dts;
      entries[i].pos = pos;
    } else {
      if (entries.size() >= max_key_entries) return;
      entries.insert(entries.begin() + static_cast<std::ptrdiff_t>(i),
                     KeyEntry{pts, dts, pos, k::no_pts});
      if (contigKey != no_entry && contigKey >= i) ++contigKey;
    }
    if (contigKey != no_entry && contigKey < entries.size() && entries[contigKey].pts < pts) {
      entries[contigKey].next_pts = pts;
  ...
  void resetContiguity() noexcept { contigKey = no_entry; }
  ...
  void noteStreamEndedInGop(std::int64_t keyPts) noexcept {
    ...
    if (i < entries.size() && entries[i].pts == keyPts)
      entries[i].next_pts = std::numeric_limits<std::int64_t>::max();
  }
  ...
  [[nodiscard]] const AVIndexEntry* getIndexKeyBefore(std::int64_t P,
                                                      const ContainerIndex& container) const
      noexcept {
    ...
    if (e == n - 1) {
      ...
      if (last == nullptr ||
          ts > last->timestamp + std::max<std::int64_t>(2 * container.frameDurationHint, 1))
        return nullptr;
    }
    for (; e >= 0; --e) {
  ...
  [[nodiscard]] std::int64_t getIndexShift() const noexcept {
    return keysHaveDts ? reorderTicks : 0;
  }
  ...
  void noteKeyframeSpan(std::int64_t from, std::int64_t to) noexcept {
    if (from == k::no_pts || to == k::no_pts || to <= from) return;
 ...
 private:
  ...
  std::size_t contigKey{no_entry};  ///< entry of the last keyframe read without a seek since (its
                                    ///< GOP extent grows)
...
[[nodiscard]] inline ContainerIndex containerIndexOf(const MediaSource& source) noexcept {
  ...
  return ContainerIndex{source.getStream(), info.index_trusted, info.frame_duration_hint};
}
