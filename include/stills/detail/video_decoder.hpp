#pragma once
// stills/detail/video_decoder.hpp — the AVCodecContext, the hardware session behind it, and the
...
// that decode loop runs (a caller asking which decoder it ended up with, see generator.hpp), so
...
class VideoDecoder {
 public:
  ...
  [[nodiscard]] std::expected<void, Error> build(const AVCodecParameters& par, AVRational timeBase,
    ...
    if (opt.max_input_pixels) cc->max_pixels = *opt.max_input_pixels;
    ...
    if (hw != nullptr) {
      cc->thread_count = 1;
    } else if (opt.decoder_threads > 0) {
      cc->thread_count = opt.decoder_threads;
    } else {
      cc->thread_count =
    ...
    if (hw != nullptr) {
      ...
      hwState->extra_frames = 0;  // avcodec_get_hw_frames_parameters already adds extra_hw_frames
    }
    ...
    if (int r = avcodec_open2(cc.get(), codecDesc, nullptr); r < 0) {
      return fail(hw != nullptr ? ErrorCode::hardware_unavailable : ErrorCode::decoder_open_failed,
                  r, std::string("avcodec_open2(") + codecDesc->name + ")");
    }
    codec = std::move(cc);
    hwActive = hw != nullptr;
    hwFramesSeen = 0;
  ...
  [[nodiscard]] AVRational getFrameRate() const noexcept {
    return codec ? codec->framerate : AVRational{0, 1};
  }
  ...
  [[nodiscard]] bool isHardwareFault(int avError) const noexcept {
    return hwActive && (hwFramesSeen == 0 || avError != k::invalid_data);
  }

  /// The three verbs of the fallback ladder. The fault is raised wherever it is observed (a send,
  /// a receive, a surface download) and consumed by the ladder in FramePipeline::image_at_impl,
  ...
  [[nodiscard]] bool didAcceptHardwareFormat() const noexcept {
    return hwState != nullptr && !hwState->declined && hwState->got_hw_format;
  }
  ...
  void publishHardware(const std::string& device) {
    publish(ActiveDecoder{codecDesc->name, true, from_av(hwType), getHardwareTypeName(hwType),
                          device, ""});
  }

  /// Publishes "software, and here is why". `reason` is what the caller wants the user to read in
  /// ActiveDecoder::fallback_reason: the automatic policy's verdict, the hardware attempts that
 ...
 private:
  void publish(ActiveDecoder a) {
    a.decoder_threads = codec ? codec->thread_count : 0;
    std::lock_guard lk(activeMutex);
    active = std::move(a);
  }
