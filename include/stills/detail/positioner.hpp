#pragma once
// stills/detail/positioner.hpp — where the decoder is, and every decision about where to send it.
//
...
// type's state — FramePipeline::after_seek() flushes the decoder, empties three frame slots,
...
///
/// The invariant the type exists for: these five never move one at a time. Before this they were
/// five members set by hand in fifteen places — `after_seek()` set all of them, `reopen()` set four
/// and omitted one, `probe_first_frame()` set three and omitted two, and the rest wrote one each.
/// All correct, by inspection rather than by construction. Here every transition is a verb with a
/// contract, so "positioned but still awaiting a keyframe from a seek that never happened" is not a
/// state a caller can reach by forgetting a line.
///
...
class Position {
 public:
  ...
  /// packet level by the landing scan. Without it the first keyframe out of the decoder only
  /// answers the request when the seek aimed at the request itself.
  [[nodiscard]] bool isLandingKnown() const noexcept { return landingKnown; }
  ...
  /// The keyframe packet just fed is the answer to this nearest-keyframe request.
  void markLandingKnown() noexcept { landingKnown = true; }
  /// Cleared wherever the decode position is thrown away: a keyframe established against a
  /// landing the pipeline has left says nothing about the next one.
  void clearLanding() noexcept { landingKnown = false; }
...
class Positioner {
 public:
  ...
  [[nodiscard]] static std::int64_t oneSecond(const StreamInfo& s) noexcept {
    return av_rescale_q(1, AVRational{1, 1}, s.time_base);
  }
  [[nodiscard]] static std::int64_t framesTicks(const StreamInfo& s, int n) noexcept {
    return s.frame_duration_hint > 0 ? n * s.frame_duration_hint : 0;
  }
  ...
  [[nodiscard]] static bool canDecodeForwardTo(std::int64_t target,
                                               const PositioningView& v) noexcept {
    return v.position.isPositioned() && !v.frontier.drained && !v.frontier.eof &&
           v.frontier.lastReceivedTs != k::no_pts && target >= v.frontier.lastReceivedTs &&
  ...
  [[nodiscard]] bool isUndecodedLandingBefore(std::int64_t target,
                                              const PositioningView& v) const noexcept {
  ...
  [[nodiscard]] std::int64_t getForwardScanLimit(const PositioningView& v) const noexcept {
    const std::int64_t three_seconds = av_rescale_q(3, AVRational{1, 1}, v.stream.time_base);
    return std::max(three_seconds, v.keys.getGopHint());
  }
  ...
  [[nodiscard]] bool isForwardCheaperThanSeek(std::int64_t target,
    ...
    if (covered && key_ts != k::no_pts && fed_ts != k::no_pts) {
      ...
      if (key_ts <= fed_ts + std::max<std::int64_t>(v.stream.frame_duration_hint, 0)) return true;
      ...
      if (v.costs.getFrameCostMs() > 0 && v.stream.frame_duration_hint > 0) {
        const double frames_to_key = static_cast<double>(key_ts - fed_ts) /
                                     static_cast<double>(v.stream.frame_duration_hint);
        return frames_to_key * v.costs.getFrameCostMs() < v.costs.getSeekCostMs();
      }
      return false;
    }
    if (v.stream.index_trusted)
  ...
  [[nodiscard]] static std::int64_t getScanAim(std::int64_t P, const PositioningView& v) noexcept {
    ...
    const std::int64_t gop_hint = v.keys.getGopHint();
    return P - margin - (gop_hint > 0 ? gop_hint : 0);
  }
  ...
  [[nodiscard]] Aim retryAfterOvershoot(std::int64_t P, std::int64_t keyPts, std::int64_t keyDts,
                                        int round, const PositioningView& v) noexcept {
    // Read after the landing, not before it: the packets it just read may have lowered the delay.
    const std::int64_t reorder_ticks = v.keys.getReorderTicks();
    const std::int64_t delay = (keyDts != k::no_pts && keyPts > keyDts) ? keyPts - keyDts
                                                                       : reorder_ticks;
    if (keyDts != k::no_pts && reorder_ticks > 0 && keyDts > P - reorder_ticks && seekBias == 0) {
      ...
      seekBias = reorder_ticks;
    }
    if (round >= 3 || backoffs >= maxBackoffs) return Aim{0, true};
    const std::int64_t target =
        keyPts - 1 - delay -
        (round > 0 ? std::max({oneSecond(v.stream), v.keys.getGopHint(), std::int64_t{1}}) * round
                   : 0);
    ++backoffs;
  ...
  [[nodiscard]] Aim nextBackoff(std::int64_t P, std::int64_t observedKey /* or k::no_pts */,
                                const PositioningView& v) noexcept {
    if (backoffStep <= 0)
      backoffStep = std::max({oneSecond(v.stream), v.keys.getGopHint(), std::int64_t{1}});
    const std::int64_t margin = framesTicks(v.stream, 2) + v.keys.getReorderTicks();
    const std::int64_t lastTarget = v.position.getSeekTarget();
    const std::int64_t anchor =
        std::min(P, observedKey != k::no_pts ? observedKey : lastTarget);
    std::int64_t earlier = anchor - backoffStep - margin;
    if (earlier >= lastTarget)
      earlier = lastTarget - backoffStep;  // always strictly earlier than last time
    if (backoffStep < std::numeric_limits<std::int64_t>::max() / 2) backoffStep *= 2;
    if (++backoffs >= maxBackoffs || earlier <= v.stream.start_pts) return Aim{0, true};
 ...
 private:
  ...
  [[nodiscard]] static bool hasDecodedNothingSinceLanding(const PositioningView& v) noexcept {
    return v.position.isPositioned() && !v.frontier.drained && !v.frontier.eof &&
           v.frontier.lastReceivedTs == k::no_pts;
  }
