...
// seekTo() or byteSeekTo(), and each increments a lifetime counter, so the count cannot drift out
// of step with the calls. A request is not a concept this type has: the owner differences the
// counter per request.
//
...
[[nodiscard]] inline DisplayTransform decodeDisplayMatrix (const std::int32_t source[9]) noexcept
{
    std::int32_t matrix[9];
    std::memcpy (matrix, source, sizeof matrix);
    DisplayTransform transform;
    const double determinant = static_cast<double> (matrix[0]) * static_cast<double> (matrix[4])
                               - static_cast<double> (matrix[1]) * static_cast<double> (matrix[3]);

    if (determinant < 0)
    {
        transform.mirrored = true;
        av_display_matrix_flip (matrix, 1, 0); // M * Fh: negates column 0, leaving a pure rotation
    }
    ...
    transform.rotation = snapped % 360;
    return transform;
}
...
public:
    ...
    enum class IoState
    {
        ...
        wasCleared, // the sticky end-of-file/error flags were cleared and the demuxer flushed
    ...
    [[nodiscard]] std::expected<void, Error> open (const Options& options)
    {
        AVFormatContext* context = avformat_alloc_context();

        if (context == nullptr) return fail (ErrorCode::outOfMemory, "avformat_alloc_context");
        context->interrupt_callback.callback = &MediaSource::interruptCallback;
        context->interrupt_callback.opaque = this;
        context->flags |= AVFMT_FLAG_GENPTS;
        DictPtr demuxerDictionary;
        {
            AVDictionary* dictionary = nullptr;

            for (const auto& [optionKey, value] : options.demuxerOptions)
                av_dict_set (&dictionary, optionKey.c_str(), value.c_str(), 0);
            demuxerDictionary.reset (dictionary);
        }

        AVDictionary* unconsumed = demuxerDictionary.release();
        ...
        const int result = avformat_open_input (&context, path.c_str(), nullptr, &unconsumed);
        demuxerDictionary.reset (unconsumed); // whatever was not consumed

        if (result >= 0) formatContext.reset (context);
        if (auto unknownKeys = getUnknownDemuxerOptions (demuxerDictionary.get()); ! unknownKeys.empty())
        {
            return fail (ErrorCode::invalidArgument,
                         "demuxerOptions: no such option in this FFmpeg build: " + unknownKeys
        ...
        if (result < 0)
        {
            ...
            if (result == libav::enoent) code = ErrorCode::fileNotFound;
            if (result == libav::invalidData) code = ErrorCode::unsupportedFormat;
            return fail (code, result, "avformat_open_input(\"" + path + "\")");
        }
        ...
        if (auto infoResult = checkDeclaredSize (options); ! infoResult) return infoResult;
        if (int infoResult = avformat_find_stream_info (formatContext.get(), nullptr); infoResult < 0)
        {
            return fail (ErrorCode::unsupportedFormat, infoResult, "avformat_find_stream_info");
        }

        const AVCodec* codec = nullptr;
        int streamIndex = av_find_best_stream (formatContext.get(), AVMEDIA_TYPE_VIDEO,
                                               options.videoStreamIndex.value_or (-1), -1, &codec, 0);

        if (streamIndex == libav::decoderNotFound)
        {
            return fail (ErrorCode::decoderNotFound, streamIndex,
                         "av_find_best_stream: no decoder for the video stream");
        }

        if (streamIndex < 0)
        {
            if (options.videoStreamIndex)
            {
                return fail (ErrorCode::invalidArgument, streamIndex,
                             "stream " + std::to_string (*options.videoStreamIndex)
                                 + " is not a decodable video stream");
            }

            return fail (ErrorCode::noVideoStream, streamIndex,
                         "av_find_best_stream: no video stream in \"" + path + "\"");
        }

        // Cover art is a "video" stream to libavformat; not to us, unless asked for.
        if ((formatContext->streams[streamIndex]->disposition & AV_DISPOSITION_ATTACHED_PIC) != 0
            && ! options.allowAttachedPictures && ! options.videoStreamIndex)
        {
            int alternative = -1;

            for (unsigned int index = 0; index < formatContext->nb_streams; ++index)
            {
                AVStream* candidateStream = formatContext->streams[index];

                if (candidateStream->codecpar->codec_type == AVMEDIA_TYPE_VIDEO
                    && (candidateStream->disposition & AV_DISPOSITION_ATTACHED_PIC) == 0
                    && avcodec_find_decoder (candidateStream->codecpar->codec_id) != nullptr)
                {
                    alternative = static_cast<int> (index);
                    break;
                }
            }
        ...
        if (options.maxInputPixels)
        {
            ...
            const std::int64_t pixels = static_cast<std::int64_t> (std::max (videoStream->codecpar->width, 0))
                                        * std::max (videoStream->codecpar->height, 0);

            if (pixels > *options.maxInputPixels)
            {
                return fail (ErrorCode::unsupportedFormat,
                             "the video stream is " + std::to_string (videoStream->codecpar->width) + "x"
                                 + std::to_string (videoStream->codecpar->height) + " = " + std::to_string (pixels)
                                 + " pixels, over Options::maxInputPixels (" + std::to_string (*options.maxInputPixels)
                                 + ")");
            }
        }
        ...
        for (unsigned int index = 0; index < formatContext->nb_streams; ++index)
        {
            if (static_cast<int> (index) != streamIndex) formatContext->streams[index]->discard = AVDISCARD_ALL;
        }

        streamInfo = StreamInfo{};
        streamInfo.index = streamIndex;
        streamInfo.timeBase = videoStream->time_base;

        if (streamInfo.timeBase.num <= 0 || streamInfo.timeBase.den <= 0)
        {
            return fail (ErrorCode::unsupportedFormat, "video stream has an invalid time base");
        }

        if (videoStream->start_time != libav::noPts)
        {
            streamInfo.startPts = videoStream->start_time;
        }
        else if (formatContext->start_time != libav::noPts)
        {
            streamInfo.startPts = av_rescale_q (formatContext->start_time, libav::timeBaseQ, streamInfo.timeBase);
        }

        if (videoStream->duration != libav::noPts && videoStream->duration > 0)
        {
            streamInfo.durationPts = videoStream->duration;
        }
        else if (formatContext->duration != libav::noPts && formatContext->duration > 0)
        {
            streamInfo.durationPts = av_rescale_q (formatContext->duration, libav::timeBaseQ, streamInfo.timeBase);
        }

        streamInfo.avgFrameRate = videoStream->avg_frame_rate;
        const AVRational frameRate =
            videoStream->avg_frame_rate.num > 0 ? videoStream->avg_frame_rate : videoStream->r_frame_rate;

        if (frameRate.num > 0 && frameRate.den > 0)
        {
            streamInfo.frameDurationHint =
                av_rescale_q (1, AVRational{ frameRate.den, frameRate.num }, streamInfo.timeBase);
        }
        ...
        streamInfo.containerSar = videoStream->sample_aspect_ratio;
        streamInfo.codecSar = videoStream->codecpar->sample_aspect_ratio;
        streamInfo.transform = readTransform (*videoStream->codecpar);
        streamInfo.ioSeekable =
        ...
        // re-opened.
        streamInfo.seekable = streamInfo.ioSeekable && (formatContext->iformat->flags & AVFMT_NOTIMESTAMPS) == 0
                              && seekFailures < maxSeekFailures;
        streamInfo.synthesizeTimestamps = (formatContext->iformat->flags & AVFMT_NOTIMESTAMPS) != 0;
        streamInfo.indexTrusted = avformat_index_get_entries_count (videoStream) > 0;
        return {};
    }
    ...
    [[nodiscard]] std::expected<void, Error> reopen (const Options& options)
    {
        const std::int64_t previousStartPts = streamInfo.startPts;
        const std::optional<std::int64_t> previousDuration = streamInfo.durationPts;
        const bool hadStream = videoStream != nullptr;
        close();

        if (auto result = open (options); ! result)
        {
            close();
            return result;
        }
        ...
        if (hadStream)
        {
            streamInfo.startPts = previousStartPts;

            if (previousDuration && (! streamInfo.durationPts || *streamInfo.durationPts < *previousDuration))
            {
                streamInfo.durationPts = previousDuration;
            }
        }
    ...
    void close() noexcept
    {
        formatContext.reset();
        videoStream = nullptr;
        codecDescriptor = nullptr;
    }

    [[nodiscard]] bool isOpen() const noexcept { return static_cast<bool> (formatContext); }
    [[nodiscard]] const StreamInfo& getStreamInfo() const noexcept { return streamInfo; }
    // The chosen video stream, or nullptr while closed. Non-const because libavformat's index
    // accessors take it that way.
    [[nodiscard]] AVStream* getStream() const noexcept { return videoStream; }
    // The decoder libavformat chose for that stream; the pipeline builds its AVCodecContext from it.
    [[nodiscard]] const AVCodec* getCodec() const noexcept { return codecDescriptor; }
    // The demuxer's name, or empty while closed or when libavformat gives none.
    [[nodiscard]] std::string getContainerName() const
    {
        if (! formatContext || formatContext->iformat == nullptr || formatContext->iformat->name == nullptr) return {};
    ...
    {
        return videoStream != nullptr && formatContext != nullptr && videoStream->start_time == libav::noPts
               && formatContext->start_time == libav::noPts;
    }

    // Anchors the time origin at a frame the caller decoded. See needsStartPtsFromFirstFrame().
    void setStartPts (std::int64_t pts) noexcept { streamInfo.startPts = pts; }
    ...
    void adoptDecoderFrameRate (AVRational frameRate) noexcept
    {
        streamInfo.frameDurationHint =
            av_rescale_q (1, AVRational{ frameRate.den, frameRate.num }, streamInfo.timeBase);
        streamInfo.avgFrameRate = frameRate;
    }
    ...
    [[nodiscard]] int seekTo (std::int64_t timestamp) noexcept
    {
        ++seekCalls;
        // min_ts = INT64_MIN, ts = max_ts: "the keyframe at or before ts", never later.
        return avformat_seek_file (formatContext.get(), streamInfo.index, std::numeric_limits<std::int64_t>::min(),
                                   timestamp, timestamp, 0);
    }

    // Positions on a packet by byte offset (containers without a trusted index). The next packet read
    // is the one at `pos`.
    [[nodiscard]] int byteSeekTo (std::int64_t bytePosition) noexcept
    {
        ++seekCalls;
        return av_seek_frame (formatContext.get(), -1, bytePosition, AVSEEK_FLAG_BYTE);
    }
    ...
    {
        const std::int64_t marginTicks = av_rescale_q (10, AVRational{ 1, 1 }, streamInfo.timeBase);
        const std::int64_t lowestTs = std::numeric_limits<std::int64_t>::min() / 2;
        const std::int64_t earlyTs =
            streamInfo.startPts - marginTicks < lowestTs ? lowestTs : streamInfo.startPts - marginTicks;
        int result = seekTo (earlyTs);

        if (result < 0 && result != libav::exitRequested) result = seekTo (streamInfo.startPts);
        return result;
    }
    ...
    {
        if (++seekFailures >= maxSeekFailures) streamInfo.seekable = false;
    }

    // --- reading ------------------------------------------------------------------------------

    // Reads the next packet of *any* stream into `pkt`. Returns 0, libav::eof, libav::exitRequested or a
    ...
    [[nodiscard]] int readPacket (AVPacket& packet) noexcept { return av_read_frame (formatContext.get(), &packet); }

    // Throws away the demuxer's buffered state. The decoder is not touched; the caller flushes it.
    void flushDemuxer() noexcept
    {
        if (formatContext) avformat_flush (formatContext.get());
    }
    ...
    void setCancelToken (const CancelToken* transform) noexcept { token = transform; }
    ...
    {
        const bool interruptSeen = std::exchange (interruptFired, false);

        if (! formatContext || formatContext->pb == nullptr) return IoState::clean;
        if (formatContext->pb->error != libav::exitRequested
            && ! (interruptSeen && formatContext->pb->eof_reached != 0))
        {
            return IoState::clean;
        }

        const bool wasCleared = tryResetIoState (*formatContext->pb);
        avformat_flush (formatContext.get());
        return wasCleared ? IoState::wasCleared : IoState::stuck;
    }
    ...
    {
        return ! formatContext || formatContext->pb == nullptr || tryResetIoState (*formatContext->pb);
    }
...
private:
    ...
    static int interruptCallback (void* callbackData) noexcept
    {
        auto* source = static_cast<MediaSource*> (callbackData);

        if (source == nullptr) return 0;
        if (source->token != nullptr && source->token->isRequested())
        {
    ...
    [[nodiscard]] static std::string getUnknownDemuxerOptions (AVDictionary* unconsumed)
    {
        std::string unknownKeys;
        const AVDictionaryEntry* entry = nullptr;

        while ((entry = av_dict_iterate (unconsumed, entry)) != nullptr)
        {
            if (optionExists (entry->key)) continue;
    ...
    // defines `key`.
    [[nodiscard]] static bool optionExists (const char* optionKey) noexcept
    {
        const AVClass* formatClass = avformat_get_class();

        if (av_opt_find (&formatClass, optionKey, nullptr, 0, AV_OPT_SEARCH_FAKE_OBJ | AV_OPT_SEARCH_CHILDREN)
            != nullptr)
            return true;
        void* iterator = nullptr;

        while (const AVClass* child = av_opt_child_class_iterate (formatClass, &iterator))
        {
            if (av_opt_find (&child, optionKey, nullptr, 0, AV_OPT_SEARCH_FAKE_OBJ | AV_OPT_SEARCH_CHILDREN) != nullptr)
                return true;
        }
    ...
    [[nodiscard]] std::expected<void, Error> checkDeclaredSize (const Options& options) const
    {
        if (! options.maxInputPixels) return {};
        int largestWidth = 0, largestHeight = 0;
        bool anyVideo = false;

        for (unsigned int index = 0; index < formatContext->nb_streams; ++index)
        {
            const AVCodecParameters& par = *formatContext->streams[index]->codecpar;
            ...
            if (pixels <= *options.maxInputPixels) return {}; // one of them might be usable
            if (pixels > static_cast<std::int64_t> (largestWidth) * largestHeight)
            {
                largestWidth = par.width;
                largestHeight = par.height;
            }
        }

        if (! anyVideo || largestWidth <= 0) return {};
        return fail (ErrorCode::unsupportedFormat,
                     "the video stream is " + std::to_string (largestWidth) + "x" + std::to_string (largestHeight)
                         + " = " + std::to_string (static_cast<std::int64_t> (largestWidth) * largestHeight)
                         + " pixels, over Options::maxInputPixels (" + std::to_string (*options.maxInputPixels) + ")");
    }
    ...
    [[nodiscard]] static DisplayTransform readTransform (const AVCodecParameters& par) noexcept
    {
        ...
        if (sideData == nullptr || sideData->size < 9 * sizeof (std::int32_t)) return DisplayTransform{};
        std::int32_t matrix[9];
        std::memcpy (matrix, sideData->data, sizeof matrix);
    ...
    // consumer build.
    [[nodiscard]] static bool tryResetIoState (AVIOContext& ioContext) noexcept
    {
#if LIBAVFORMAT_VERSION_MAJOR <= 63 && ! defined(STILLS_FORCE_UNVERIFIED_LIBAVFORMAT)
        ioContext.eof_reached = 0;
        ioContext.error = 0;
        return true;
#else
        (void)ioContext;
    ...
    std::string path;
    FormatCtxPtr formatContext;
    AVStream* videoStream{ nullptr };
    const AVCodec* codecDescriptor{ nullptr }; // the decoder av_find_best_stream picked, not a context
    StreamInfo streamInfo;
